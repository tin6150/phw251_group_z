...
```{r massage mortality data, echo=T, include=T}
...
```{r massage funding data, echo=T, include=T}
...
```{r spot checks, echo=T, include=T}
...
```{r massage demographics data, echo=T, include=T}
...
```{r demographics data complete set high rental, old age, echo=T, include=T}
...
```{r demographics data RURAL set only, high rental, old age, echo=T, include=T}
...
```{r tmp-double-whammy, include=T, echo=T}
...
```{r dem relaxed criteria: old age, high rental in rural standard, echo=T, include=T}
...
```{r discussion-use-only,  include=T, echo=T}
...
```{r calc chronic mortality rate, echo=T, include=T}
...
```{r join tables, echo=T, include=T}
...
```{r spot_check_mortality_from_source_data, echo=T, include=T }
...
```{r spot_check_mortality, echo=T, include=T }
...
```{r draft-viz-for-discussion-only, echo=T, include=T }
...
[National Rural Development Partnership](https://www.washingtonpost.com/politics/the-federal-definition-of-rural--times-15/2013/06/08/a39e46a8-cd4a-11e2-ac03-178510c9cc0a_story.html), are counties with population density below 20 person per square miles.  
...
subplot(MedianAgePlot, RentPlot, Funding_Construction_Plot, shareY=F) %>%
  layout(title = "Fig 1: Age, Rent, & Funding Characteristics of Rural Counties",
         ...
         yaxis3 = list(title = "In Construction Funding($)"))
```
...
across the 11 rural counties in CA (as defined by [National Rural Development Partnership](https://www.washingtonpost.com/politics/the-federal-definition-of-rural--times-15/2013/06/08/a39e46a8-cd4a-11e2-ac03-178510c9cc0a_story.html), 20 person/sq.mile)
...
```{r, New fig2 which embed table2, include=T, echo=F}

#### tmp, see if i like it first
#### trying to combine fig2 + table2
#### #m6delta

# this was from ~line 98, move back up  ++FIXME++
####chronic_by_county__M6 = chronic_mortality_data %>%
chronic_5yrAvg__M6 = chronic_mortality_data %>%
  filter(  Strata == "Total Population") %>%
  filter(  Geography_Type == "Occurrence" ) %>%
  select( -Geography_Type ) %>% # Geo type restricted to "Occurrence"
  filter( Year >= 2016 )     %>%   # 2016-2020 = 5 years
  group_by( County,Cause_Desc ) %>%          #m6delta new grouping
  summarize( sum_count5 = sum(Count),
             avg_count  = sum(Count) / 5   # this avg over the 5 years
             #County, Cause
             ) 

# from line463, maybe replace it with  this new code
demographics_chronic_M6 = left_join( demographics_data, 
                                  chronic_5yrAvg__M6,       ##chronic_by_county, 
                                  by = "County"      ) %>% 
  mutate( pct     = ( avg_count / pop2012 ) * 100 ) %>%
  mutate( per100k = ( avg_count / pop2012 ) * 100000 ) %>%
  group_by( County )   %>%
  summarize( allCauseAvgCount = sum(per100k)  , # this is the total height of the bar here, ie across all disease  
             per100k,
             County,Cause_Desc,avg_count,pop2012  #,All_Causes_sum ,Year
             )  


chronic_focus_countiesM6 = inner_join(
    demographics_chronic_M6,  
    rural_counties,
    by = "County"                   ) 


fig_mortalityM6 = plot_ly( 
    data   = chronic_focus_countiesM6 ,
    x      = ~County,
    y      = ~per100k,
    color  = ~Cause_Desc,     #m6delta   
    type   = 'bar'
  ) %>%
  add_trace(
            y = ~ allCauseAvgCount,
            ...
            text = ~paste( 
                           "Mortality Rate per 100k: "   ,  round(allCauseAvgCount, 1),   # not per100k col anymore
                           "<BR>2012 Population: "       ,  pop2012, 
                           "<BR>Population per sq.mile: ",  round(pop12_sqmi,2) 
            ),
            type      = "scatter"  ,
            mode      = "markers",
            showlegend = F,
