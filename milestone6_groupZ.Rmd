...
```
##### new Figure 2 with embeded disease break down from Table 2.

```{r, trying to combine fig2 + table2, include=T, echo=F}
...
chronic_5yrAvg__M6 = chronic_mortality_data %>%
  ...
  select( -Geography_Type ) %>% # Geo type restricted to "Occurrence"
  filter( Year >= 2016 )     %>%   # 2016-2020 = 5 years
  group_by( County,Cause_Desc ) %>%                      #m6delta add:  Cause_Desc  # no more year
  summarize( sum_count5 = sum(Count),
             avg_count  = sum(Count) / 5   # this avg over the 5 years
             #County, Cause
             ) %>%  #1210  # oh no need, did... allCauseAvgCount
  group_by( County ) #%>%
  #summarize( All_Causes_sum = sum(_count5),        # this is a 5 year count across all diseases for each county used as total by Fig 2.
  #           Year,Cause_Desc,sum_count5,avg_count )




# line463 change to this
## ++FIXME++   no, this is broken.  the sum would be over 5 years... for all diseases...  above need to tune to group_by disease before doing sum/count...  ...maybe fixed...
demographics_chronic_M6 = left_join( demographics_data, 
                                  chronic_5yrAvg__M6,       ##chronic_by_county, 
                                  by = "County"      ) %>% 
  mutate( pct     = ( avg_count / pop2012 ) * 100 ) %>%
  mutate( per100k = ( avg_count / pop2012 ) * 100000 ) %>%
  #group_by( County,Year )   %>%                      ## fidddle here to get right height ++
  group_by( County )   %>%
  summarize( allCauseAvgCount = sum(per100k)  , #dont /5 ## should get the total height of the bar here -- ie across all disease  
             per100k,
...
## would  no longer need this, use new block next ++FIXME++
chronic_focus_countiesM6_xx = inner_join(
    demographics_chronic,
    rural_counties,
    by = "County"                   ) %>%
  mutate( ctyColor = case_when(
...
fig_mortalityM6 = plot_ly( 
    ...
    x      = ~County,
    y      = ~per100k,
    color  = ~Cause_Desc,     #m6delta    ##++ try to get Cause_Desc instead 
    type   = 'bar'
    #,               
    #text   = ~paste( round( per100k, 0) ),
    #showlegend = F,
  ...
  add_trace(
            ...
            marker = list( color = "lightgrey", size=10,
                           line  = list( color="grey", width=2 ) ),
            #marker = list(color = ~ctyColor),
            text = ~paste( #"Total Average Yearly Deaths:",  avg_count, 
