...
```{r, trying to combine fig2 + table2, include=F, echo=FALSE}
...
chronic_5yrAvg__M6 = chronic_mortality_data %>%
  ...
  select( -Geography_Type ) %>% # Geo type restricted to "Occurrence"
  filter( Year >= 2016 )     %>%   # 2016-2020 = 5 years
  group_by( Year,County,Cause ) %>%                      #m6delta Year instead of County
...
demographics_chronic_M6 = left_join( demographics_data, 
  ...
  group_by( County )   %>%
  summarize( allCauseAvgCount = sum(per100k) / 5 ,  ## should get the total height of the bar here -- ie across all disease
             per100k,
             County,Year,Cause,avg_count,pop2012
             )  
...
fig_mortalityM6 = plot_ly( 
  ...
  # can there be 2 traces?   but they are not overlay, now height is doubled.   may not get what i want even if get bullet height right ++FIXME++
  add_trace(
            #name = 'Fig 2: Chronic Mortality Rates by Counties',
            x      = ~County,
            y      = ~allCauseAvgCount,
