...
__(I (Tin) am fuzzy what's needed here.  no code?  just need to describe a few variables?)__
...
table_1 = mortality_data %>% 
  filter( Cause_Desc %in% chronic_desc ) #%>%
  
table_2 = mortality_data %>% 
  filter( Strata == "Total Population") %>%
  group_by( County, Cause_Desc ) %>%
