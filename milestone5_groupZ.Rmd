...
and have meadian age greter than the state-wide average.
...
focus_table = viz_focus %>%
  ...
  mutate( highliteCounty = case_when(
    County %in% selected_counties ~ TRUE, 
    TRUE                          ~ FALSE
  ) ) %>%
...
datatable( 
  ...
  formatStyle(4,backgroundColor = styleInterval(c(avg_rent_ratio),c('white',grey_blue)))   # highlite if rental ratio > state avg, none in here fit this criteria
...
```{r viz: funding for select counties, include=F, echo=TRUE }
