...
demographics_chronic = left_join( demographics_data, 
  ...
  mutate( per100k = ( sum_count / pop2012 ) * 100000 ) 
...
# this where mortality rate was calc (as pct)
#demographics_chronic = left_join( demographics_data,
#                                  chronic_by_county,
#                                  by = "County"      ) %>%
#  mutate( pct = ( sum_count / pop2012 ) * 100 )
...
Number of Chronic cases for each county is actually the average number of yearly cases between 2016-2022.
...
focus_table = viz_focus %>%
  ...
  rename(
    ...
    `Total Pop`            = pop2012,
    `Num of Chronic cases` = sum_count,
    `Median Age`           = med_age,
...
fig_mortality = plot_ly( 
  data = chronic_focus_counties ,
    ...
    add_trace(
            ...
            hoverinfo = "text",
            #text = ~paste(round(per100k, ), "%" ),
            text = ~paste("Total Cases:", sum_count, "2012 Population: ", pop2012, "Mortality Rate: ", round(per100k, 1)  ),  
            type = 'bar') %>%
   layout(
