...
`++TEMP NOTE++`
`I (Tin) change filter from Year>2016 to Year>=2016 to have a proper 5 year bracket,`
`results not significatly different than Milestone 4`


## Rural counties with high median age and renter to owner ratio 


Table 1 shows CA counties that are rural
(per National Rural Development Partnership's definition), 
and have median age grater than the state-wide average.
...
datatable( 
  ...
  formatStyle(3,backgroundColor = styleInterval(c(avg_age),              c('white',grey_blue))) %>%  # highlite = older than state avg
  formatStyle(5,backgroundColor = styleInterval(c(avg_chronic_mortality),c('white',grey_blue))) %>%  # highlite = higher chronic mortality than state avg
  formatStyle(4,backgroundColor = styleInterval(c(avg_rent_ratio),       c('white',grey_blue)))      # highlite = rental ratio > state avg, none in here fit this criteria
...
chronic_focus_counties = inner_join(
  ...
  mutate( ctyColor = case_when(
    ...
    TRUE                 ~ grey_blue     # "rgb(187, 216, 228)"
  ) )


fig_mortality = plot_ly( data = chronic_focus_counties ) %>%
    add_trace(
            x = ~County,
            y = ~pct,
            #name = 'Fig 1: Chronic Mortality Rates by Counties',
            marker = list(color = ~ctyColor),
            hoverinfo = "text",
            text = ~paste(round(pct, 1), "%" ),  
            type = 'bar') %>%
   layout(
            title = "Fig 1: Chronic Mortality Rates For California's Rural Counties", 
...
The following line charts display characteristics of interest in more detail for
rural counties. For each county, the first two subplots depict median age 
and rent:own ratio, respectively. The third subplot depicts the funding amount 
each county received on projects with a status of "in construction" as of August 
2022. We wanted to explore this variable in addition to funding for projects 
in closure as it's an important factor in deciding who needs funding most 
urgently. 
...
subplot(MedianAgePlot, RentPlot, Funding_Construction_Plot, shareY=F) %>%
  layout(title = "Fig 2: Age, Rent, & Funding Characteristics of Rural Counties",
...
# This was left out in Milestone 4, probably no need for it here either.


```{r viz: funding for select counties, echo=F, include=T }
...
From this, we got a list with 8 counties in Table 1, which warrant our attention for further intervention.
we observe that this list of 8 counties, all had a zero dollar amount for the HCAI funding at the "In Closure" state.


Then, in Figure 2, we ranked the 11 rural California counties by decreasing chronic mortality rate.
We found that the top 5 counties of Siskiyou, Inyo, Marioposa, Plumas and Modoc were also found to be top counties in the high median age and large renter to owner ratio in Table 1 above.  
...
In conclusion, we recommends that the 5 counties of Siskiyou, Inyo, Marioposa, Plumas and Modoc get priority funding for the next round of projects.  
